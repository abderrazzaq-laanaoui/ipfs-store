...
class ImageStore {
  ...
  async connect(ipfs, options = {}) {
    ...
    this.feed = await this.odb.open("testOrbit01", {
      ...
      localOnly: false,
      type: "docstore", //eventlog,feed,keyvalue,docstore,counter
    ...
    await this.loadIamges();
    this.isOnline = true;
  }

  addImageToStore = (entry) => {
    if (this.posts.filter((e) => { return e.hash === entry.hash; }).length === 0) {
      ...
      console.log("adding newImageObj to images store", newImageObj);
      this.posts.push(newImageObj);
    }
  };
  ...
  async loadIamges() {
    ...
    this.feed.events.on("replicated", async (dbAddress, count, newFeed, d) => {
      ...
      newFeed.all.map(this.addImageToStore);
    ...
    this.feed.events.on("ready", (dbAddress, feedReady) => {
      ...
      // this.feed = feedReady
      if (this.feed !== undefined) this.feed.all.map(this.addImageToStore);
   ...
   */
  async createNewPost() {
    console.log("creating new postFeed", this.currentImage.subject)
    const postsFeed = await this.odb.feed(this.currentImage.subject, {
      accessController: { type: "orbitdb", write: [this.odb.identity.id] },
    })

    const p = {
      subject: this.currentImage.subject,
      body: this.currentImage.body,
      createdAt: this.currentImage.createdAt ? this.currentImage.createdAt : new Date().getTime(),
  ...
  async removePost() {
    const filteredData = this.posts.filter((item) => {
      return item.hash !== this.currentImage.hash
    });

    this.posts.replace(filteredData);
    const hash = await this.feed.remove(this.currentImage.hash);
    return hash;
  }
  ...
  //     const imagePost = this.odb.stores[nextAddress] || (await this.odb.open(nextAddress));
  //     await imagePost.load();
  //     this.currentFeed = imagePost; 
  //   }else console.log('odb not loaded')

  // }

  async joinImageItem(address) {
    console.log("joinImageItem - loading address", address);
    if (this.odb) {
      const ourImage = this.posts.filter((item) => { return item.address === address })
      this.currentImage = ourImage.length > 0 ? ourImage[0] : undefined
      try {
        const imageItem =
          this.odb.stores[address] || (await this.odb.open(address));
        await imageItem.load();
        this.currentFeed = imageItem
      } catch (ex) {
  ...
  async addFile(address, source) {
    ...
    return await this.addImage(address, data);
  }

  async addImage(address, data) {
    console.log("adding data to db on address", address);
    const imageItem = this.odb.stores[address] || (await this.odb.open(address));
    if (imageItem) {
      const hash = await imageItem.add(data);
      console.log("got hash", hash);
      await imageItem.load();
      this.currentFeed = imageItem;
      console.log("blogPost feed loaded", this.currentFeed);
      return imageItem.get(hash);
    }
    return;
  }
}

const store = global.store = new ImageStore();
const ipfs = IPFS.create({ silent: true });

store.connect(ipfs).then(() => {
